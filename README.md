...
This is useful when working with one dataset at a time. For example, if only the Thyroid Recurrence dataset has been downloaded, the command below should return `OK` only for that dataset:

```bash
python src/scan_raw.py --dataset thyroid_recurrence
```

For image datasets, the scanner reports:
...
For the tabular dataset, the scanner reports:

```text
number of files
detected CSV / Excel file
table shape
column names
```

The scan summary is saved locally to:
...
The `artifacts/` directory is intended for local generated outputs and should not be committed.
...
2. Verify downloaded raw data with src/scan_raw.py.
3. Prepare train / validation / test splits.
4. Run baseline experiments.
5. Run stability experiments.
6. Run quality control and leakage checks.
7. Generate leakage-free splits where needed.
8. Re-run selected baselines on leakage-free splits.
9. Build final summaries and registry tables.
```
